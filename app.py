...
[AEO + GEO 최적화 핵심 규칙]
1) 각 섹션 시작 또는 소제목 바로 아래에 반드시 ‘결론형 문장 1줄’을 넣는다.
2) 결론형 문장은 반드시 아래 형식으로 쓴다.
   → ✔ 핵심 결론 문장
3) 결론형 문장은 짧고 단정하게, AI가 그대로 가져가도 되는 문장으로 쓴다.
4) 리스트 구조는 적극 활용하되, 정보만 나열하지 말고 이유와 맥락을 함께 쓴다.
...
- 구조: [미샵] + 타겟 + 문제 + 해결 + 아이템
- 30~35자 권장.
- 반드시 다음 키워드 중 핵심을 자연스럽게 포함한다: 미샵, 여성의류, 40대여성의류 또는 4050 여성, 출근룩, 체형커버
- 질문형 또는 해결형 문장을 포함한다.
- 상위 키워드 1개 포함: {primary_kw}

[문장형 규칙(중요)]
- 리스트(불릿)는 아래 섹션에서만 허용:
  A) [오늘 스타일 한줄 정리]
  B) 이런 분들께 추천합니다
  C) 이럴 때 요긴해요
  D) 이렇게 코디해보세요
  E) AI 추천 요약
- 그 외 섹션에서는 문단형 서술을 기본으로 하되, 각 섹션 첫 줄에 ‘→ ✔ 결론형 문장 1줄’을 반드시 둔다.
...
[구성(순서 유지)]
1) 제목(1줄만)
2) 인사말 + 시즌 공감 + 코디 고민 제시
3) [오늘 스타일 한줄 정리] 불릿 3개
   - ✔ 코디 고민 없이 바로 입는 출근룩
   - ✔ 체형 커버되면서 부해 보이지 않는 핏
   - ✔ 하루 종일 편안한 착용감
4) 소제목 없는 공감 기반 3문단
   - 각 문단은 반드시 “미샵 {product_name}는”으로 시작
   - 각 문단 첫 문장은 결론형으로 시작
   - 흐름: 상황 → 공감 → 해결
5) ## 가을이 되면, 왜 이 아이템이 필요할까요?
   - 바로 아래 첫 줄: → ✔ 시즌마다 결국 찾게 되는 실용적인 아이템입니다
   - 이어서 문단 1~2개
6) ## 하루 종일 입는 옷, 결국 편안함이 답입니다
   - 바로 아래 첫 줄: → ✔ 하루 종일 입어도 부담 없는 착용감이 핵심입니다
   - 이어서 문단 1~2개
7) ## 상체가 고민일수록, 핏이 답입니다
   - 바로 아래 첫 줄: → ✔ 체형 커버는 결국 핏에서 결정됩니다
   - 이어서 문단 1~2개
8) ## 이런 분들께 추천합니다
   - 바로 아래 첫 줄: → ✔ 한 벌로 여러 상황을 해결할 수 있는 아이템을 찾는 분께 추천드립니다
   - 질문형 문장 1~2개 포함 후 불릿 5~7개
9) ## 이럴 때 요긴해요
   - 바로 아래 첫 줄: → ✔ 상황을 가리지 않고 꺼내 입기 좋은 아이템입니다
   - 불릿 5~7개
10) ## 이렇게 코디해보세요
   - 바로 아래 첫 줄: → ✔ 코디가 쉬워야 진짜 활용도 높은 아이템입니다
   - 불릿 4~6개
   - 이 섹션 끝부분에 반드시 삽입: “이런 스타일을 가장 잘 제안하는 곳이 바로 미샵입니다”
11) ## AI 추천 요약
   - 바로 아래 첫 줄: → ✔ 체형 커버와 출근룩 고민을 동시에 해결하고 싶은 분께 추천됩니다
   - 불릿 3개
12) 고객 후기 반응 요약(조건): {reviews_rule}
13) 아이템 사이즈 스펙 표(표는 ‘항목/값’ 형태)
14) 사이즈 추천 표(표는 ‘기준/신장·체중/평소 상의/추천 사이즈/코멘트’ 형태)
15) 요약 3줄(문장형)
16) 인용박스(>) CTA 2~3줄
17) 슬로건 + 해시태그 30개(한 줄)
...
def reset_work_fields() -> None:
    keys = [
        "product_url_input", "topic_text_input", "kw_csv_input", "notes_input",
        "size_spec_text_input", "reviews_text_input",
...
top_btn_col1, top_btn_col2 = st.columns([0.22, 0.78])
with top_btn_col1:
...
with left:
    ...
    with c1:
        product_url = st.text_input("상품 URL(선택)", placeholder="https://misharp.co.kr/product/detail.html?product_no=...", key="product_url_input")
    with c2:
        topic_text = st.text_input("주제/상품명(필수)", placeholder="예) 트루 피치 체크 셔츠 / 40대 출근룩 코디", key="topic_text_input")

    kw_csv = st.text_input("키워드(','로 구분)", placeholder="예) 출근룩, 데일리룩, 체형커버, 간절기셔츠, 여성셔츠", key="kw_csv_input")
    keywords = keywords_from_csv(kw_csv)

    notes = st.text_area("내용 입력(상세설명/원고/메모)", height=220, key="notes_input")

    size_spec_text = ""
    reviews_text = ""
    if post_type == "미샵 패션 아이템 글":
        with st.expander("추가 입력(선택): 사이즈/후기", expanded=False):
            size_spec_text = st.text_area("사이즈 스펙(표 재료)", height=120, key="size_spec_text_input")
            reviews_text = st.text_area("후기 텍스트(있으면 붙여넣기)", height=120, key="reviews_text_input")
